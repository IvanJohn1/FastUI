    ...
    return [
        c.Page(
            components=[
                ...
                c.ModelForm(model=UserAdd, submit_url='/api/user')
            ]
        )
    ]
    ...
    return [
        c.Page(
            components=[
                ...
                c.Link(components=[c.Text(text='Back')], on_click=BackEvent()),
                c.Details(data=user),
                c.Button(text="Удалить пользователя",
                         on_click=PageEvent(name="delete-user")),
                c.Form(
                    submit_url="/api/user/delete",
                    form_fields=[
                        c.FormFieldInput(name='id', title='',
                                         initial=user_id, html_type='hidden')
                    ],
                    footer=[],
